...
Испалиндром bool static(WordDescriptor word) {
    слово.конец--;

    пока (слово.начало < слово.конец) {
        если (*слово.начало != *слово.конец) {
            возвращает false;
        }
...
Количество палиндромов int(char* string) {
    0 = count int;
    строка= точка поиска * символ;
    word WordDescriptor;

    while (getWord(точка поиска, &word)) {
        if (испалиндром(слово)) {
            ++количество;
        }

        слово = точка поиска.конец;
    }

    количество возвращений;
}

mixWords пустота(символ* слева, символ* справа, символ* пункт назначения) {
    word1 WordDescriptor, word2;
...
void getStrFromWordsNotEqualToTheLast(char* string) {
    ...
    while (getWord(sourceStart, &currentWord)) {
        ...
        currentWord = sourceStart.end;
    }

    *(destinationStart - (destinationStart != строка)) = '\0';
}
...
Получаем слово beforefirstwordinbothstr WordBeforeFirstWordWithAReturnCode(символ* строка, символ* слово, описание слова* wordDst) {
    Предыдущее слово WordDescriptor;

    if (getWord(string, &previousWord)) {
        if (!найдено слово(слово, предыдущее слово)) {
            Предыдущее слово = строка.конец;
            Следующее слово WordDescriptor;

            while (getWord(строка, &следующее слово)) {
                if (найдено слово(word, nextWord)) {
                    Предыдущее слово= wordddst *;
                    WORD_FOUND возвращает;
                }

                Следующее слово = предыдущее слово;
                Предыдущее слово = строка.конец;
            }

            NOT_FOUND_A_WORD_WITH_A возвращает;
        }

        FIRST_WORD_WITH_A возврат;
    }

    EMPTY_STRING возвращает;
}
